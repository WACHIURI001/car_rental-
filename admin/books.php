...
<div class="container-fluid">
	...
	<div class="col-lg-12">
					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								...
								$books = $conn->query("SELECT b.*,c.model,c.brand,c.transmission_id,c.engine_id,c.price FROM books b inner join cars c on c.id = b.car_id ");
								while($row=$books->fetch_assoc()):
									
								?>
								<?php
		$diff = abs(strtotime($row['dropoff_datetime']) - strtotime($row['pickup_datetime']));
		$years = floor($diff / (365*60*60*24));
		$months = floor(($diff - $years * 365*60*60*24) / (30*60*60*24));
		$days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
		$totalPrice = $row['price'] * $days;
		...
		<?php
		$sql = "SELECT  SUM(tprice) from books";
$result = $conn->query($sql);
//display data on web page
while($row = mysqli_fetch_array($result)){
    echo " Total income: ". $row['SUM(tprice)'];
    echo "<br>";
}
		?>
	</div>	
